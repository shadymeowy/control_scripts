...
async def run():
    ...
    drone = System()
    find, stop = image_process.make_object_finder(camera_model.w_px, camera_model.h_px, debug=False)
    ...
    await drone.offboard.set_position_ned(PositionNedYaw(0.0, 0.0, -3.0, yaw0))'''
    ...
    async for position in drone.telemetry.attitude_euler():
        ...
        if im_val is not None:
            x, y = im_val
            step_right, step_front = calculate_step(camera_model, x, y)
            next_position = calculate_next_position(position, yaw0, step_right, step_front)
            print(next_position)
            # await drone.offboard.set_position_ned(PositionNedYaw(next_position[0], next_position[1], -3.0, yaw0))
        else:
            # await drone.offboard.set_position_ned(PositionNedYaw(position[0], position[1], -3.0, yaw0))
            print('Stays same')
